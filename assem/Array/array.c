...
Array *Enlarge(Array *array, int new_size)
{
    ...
    {
        return (array);
    Array *newArray = createArray(new_size);
    for (int i = 0; i < array->length; i++)
    {
        newArray->arr[i] = array->arr[i];
    }
...
Array *merge(Array *arr1, Array *arr2)
{
    ...
    Array *mergedArray = createArray(size);

    for (int i = 0; i < arr1->length; i++)
    {
        mergedArray->arr[i] = arr1->arr[i];
        mergedArray->length++;
    }

    for (int i = 0; i < arr2->length; i++)
    {
        mergedArray->arr[arr1->length + i] = arr2->arr[i];
        mergedArray->length++;
    }
